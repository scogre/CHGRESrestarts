...
!------------------------------------------------------------------------------
 ...
 real                               :: num_days, num_days_rec1, rinc(5)
 real                               :: weight_rec1, weight_rec2

 if (trim(field) == 'facsf') filename = "/" // trim(cres_target_grid) // ".facsf." // trim(tiles_target_grid(tile)) // ".nc"
 if (trim(field) == 'maximum_snow_albedo') filename = "/" // trim(cres_target_grid) // ".mxsno." // trim(tiles_target_grid(tile)) // ".nc"
 if (trim(field) == 'slope_type') filename = "/" // trim(cres_target_grid) // ".slope." // trim(tiles_target_grid(tile)) // ".nc"
 if (trim(field) == 'soil_type') filename = "/" // trim(cres_target_grid) // ".soilt." // trim(tiles_target_grid(tile)) // ".nc"
 if (trim(field) == 'substrate_temperature') filename = "/" // trim(cres_target_grid) // ".tbot." // trim(tiles_target_grid(tile)) // ".nc"
 if (trim(field) == 'vegetation_greenness') filename = "/" // trim(cres_target_grid) // ".green." // trim(tiles_target_grid(tile)) // ".nc"
 if (trim(field) == 'vegetation_type') filename = "/" // trim(cres_target_grid) // ".vegt." // trim(tiles_target_grid(tile)) // ".nc"
 if (trim(field) == 'visible_black_sky_albedo') filename = "/" // trim(cres_target_grid) // ".albedo." // trim(tiles_target_grid(tile)) // ".nc"
 if (trim(field) == 'visible_white_sky_albedo') filename = "/" // trim(cres_target_grid) // ".albedo." // trim(tiles_target_grid(tile)) // ".nc"
 if (trim(field) == 'near_IR_black_sky_albedo') filename = "/" // trim(cres_target_grid) // ".albedo." // trim(tiles_target_grid(tile)) // ".nc"
 if (trim(field) == 'near_IR_white_sky_albedo') filename = "/" // trim(cres_target_grid) // ".albedo." // trim(tiles_target_grid(tile)) // ".nc"
 ...
 if (num_times > 1) then
   ...
   if (bound2 /= 1) then
     ...
     print*,'num days between bounds ', num_days
     num_days_rec1 = rinc(1) - float(days_since(bound1))
     print*,'num days from rec 1 ', num_days_rec1
