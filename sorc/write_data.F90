...
!---------------------------------------------------------------------------
 ...
 integer                        :: id_dz_bottom, id_dz_top
 integer                        :: id_dz_right, id_dz_left
 ...
 integer                        :: id_clda_bottom, id_clda_top
 integer                        :: id_clda_right, id_clda_left
...
!--- open the file
   ...
   error = nf90_def_var(ncid, 'dz_bottom', NF90_FLOAT, &
                             (/dim_lon, dim_halo, dim_lev/), id_dz_bottom)
   call netcdf_err(error, 'DEFINING DZ_BOTTOM')

   error = nf90_def_var(ncid, 'w_top', NF90_FLOAT, &
                             (/dim_lon, dim_halo, dim_lev/), id_dz_top)
   call netcdf_err(error, 'DEFINING DZ_TOP')

   error = nf90_def_var(ncid, 'w_right', NF90_FLOAT, &
                             (/dim_halo, dim_lat, dim_lev/), id_dz_right)
   call netcdf_err(error, 'DEFINING DZ_RIGHT')

   error = nf90_def_var(ncid, 'w_left', NF90_FLOAT, &
                             (/dim_halo, dim_lat, dim_lev/), id_dz_left)
   call netcdf_err(error, 'DEFINING DZ_LEFT')
   ...
   if (gfdl_mp) then
     ...
     error = nf90_def_var(ncid, 'clda_bottom', NF90_FLOAT, &
                             (/dim_lon, dim_halo, dim_lev/), id_clda_bottom)
     call netcdf_err(error, 'DEFINING CLDA_BOTTOM')

     error = nf90_def_var(ncid, 'clda_top', NF90_FLOAT, &
                             (/dim_lon, dim_halo, dim_lev/), id_clda_top)
     call netcdf_err(error, 'DEFINING CLDA_TOP')

     error = nf90_def_var(ncid, 'clda_right', NF90_FLOAT, &
                             (/dim_halo, dim_lat, dim_lev/), id_clda_right)
     call netcdf_err(error, 'DEFINING CLDA_RIGHT')

     error = nf90_def_var(ncid, 'clda_left', NF90_FLOAT, &
                             (/dim_halo, dim_lat, dim_lev/), id_clda_left)
     call netcdf_err(error, 'DEFINING CLDA_RIGHT')
...
! DZ
 ...
 print*,"- CALL FieldGather FOR TARGET GRID DZ FOR TILE: ", tile
 call ESMF_FieldGather(dz_target_grid, data_one_tile_3d, rootPet=0, tile=tile, rc=error)
 if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
      call mpi_abort
 ...
 if (localpet == 0) then
   call flip(data_one_tile_3d, i_target, j_target, lev_target)
   dum3d_top(:,:,:) = data_one_tile_3d(i_start_top:i_end_top,j_start_top:j_end_top,:)
   error = nf90_put_var( ncid, id_dz_top, dum3d_top)
   call netcdf_err(error, 'WRITING DZ TOP' )
   dum3d_bottom(:,:,:) = data_one_tile_3d(i_start_bottom:i_end_bottom,j_start_bottom:j_end_bottom,:)
   error = nf90_put_var( ncid, id_dz_bottom, dum3d_bottom)
   call netcdf_err(error, 'WRITING DZ BOTTOM' )
   dum3d_left(:,:,:) = data_one_tile_3d(i_start_left:i_end_left,j_start_left:j_end_left,:)
   error = nf90_put_var( ncid, id_dz_left, dum3d_left)
   call netcdf_err(error, 'WRITING DZ LEFT' )
   dum3d_right(:,:,:) = data_one_tile_3d(i_start_right:i_end_right,j_start_right:j_end_right,:)
   error = nf90_put_var( ncid, id_dz_right, dum3d_right)
   call netcdf_err(error, 'WRITING DZ RIGHT' )
 endif
...
!  Graupel
   ...
   print*,"- CALL FieldGather FOR TARGET GRID CLOUD AMT FOR TILE: ", tile
   call ESMF_FieldGather(clda_target_grid, data_one_tile_3d, rootPet=0,tile=tile, rc=error)
   if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__))&
      call mpi_abort
   ...
   if (localpet == 0) then
     call flip(data_one_tile_3d, i_target, j_target, lev_target)
     dum3d_top(:,:,:) =data_one_tile_3d(i_start_top:i_end_top,j_start_top:j_end_top,:)
     print*,"-SGSGSGSGSGSGSGSGS... cloud top=",data_one_tile_3d
     error = nf90_put_var( ncid, id_clda_top, dum3d_top)
     call netcdf_err(error, 'WRITING CLDA TOP' )
     dum3d_bottom(:,:,:) =data_one_tile_3d(i_start_bottom:i_end_bottom,j_start_bottom:j_end_bottom,:)
     error = nf90_put_var( ncid, id_clda_bottom, dum3d_bottom)
     call netcdf_err(error, 'WRITING CLDA BOTTOM' )
     dum3d_left(:,:,:) =data_one_tile_3d(i_start_left:i_end_left,j_start_left:j_end_left,:)
     error = nf90_put_var( ncid, id_clda_left, dum3d_left)
     call netcdf_err(error, 'WRITING CLDA LEFT' )
     dum3d_right(:,:,:) =data_one_tile_3d(i_start_right:i_end_right,j_start_right:j_end_right,:)
     error = nf90_put_var( ncid, id_clda_right, dum3d_right)
     call netcdf_err(error, 'WRITING CLDA RIGHT' )
   endif
...
! V-WINDS 'W'
 ...
 integer                          :: id_dz, id_u, id_v
 integer                          :: id_t, id_delp, id_phis, id_clda
...
!--- define dimension
 ...
 !--- define field
     ...
     error = nf90_def_var(ncid, 'u', NF90_DOUBLE,(/dim_lon,dim_latp,dim_lev,dim_time/),id_u)
     error = nf90_put_att(ncid, id_u, "long_name", "u wind")
     error = nf90_put_att(ncid, id_u, "units", "m/s")
     call netcdf_err(error, 'WRITING U' )

     error = nf90_def_var(ncid, 'v', NF90_DOUBLE,(/dim_lonp,dim_lat,dim_lev,dim_time/),id_v)
     error = nf90_put_att(ncid, id_v, "long_name", "v wind")
     error = nf90_put_att(ncid, id_v, "units", "m/s")
     call netcdf_err(error, 'WRITING V' )

     error = nf90_def_var(ncid, 'DZ', NF90_DOUBLE, (/dim_lon,dim_lat,dim_lev,dim_time/), id_dz)
     error = nf90_put_att(ncid, id_dz, "long_name", "DZ")
     error = nf90_put_att(ncid, id_dz, "units", "none")
     call netcdf_err(error, 'WRITING DZ' )
...
!
!   if (localpet == 0) then
!     allocate(dz(i_target_out,j_target_out,lev_target))
...
!     call flip(data_one_tile_3d, i_target, j_target, levp_target)
!     dum3d(:,:,:) = data_one_tile_3d(i_start:i_end,j_start:j_end,:)
!     dz=dum3d(:,:,2:levp_target)-dum3d(:,:,1:lev_target)
!     error = nf90_put_var( ncid, id_dz, dz)
!     call netcdf_err(error, 'WRITING DZ RECORD' )
!   endif
!
!   deallocate(dz, dum3d, data_one_tile_3d)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!  d-height
   ...
   if (localpet == 0) then
     print*,"SGDZSGDZSGDZ localpet!!!=",localpet
     call flip(data_one_tile_3d, i_target, j_target, lev_target)
     dum3d(:,:,:) = data_one_tile_3d(i_start:i_end,j_start:j_end,:)
     error = nf90_put_var( ncid, id_dz, dum3d)
     print*,"SGDZSGDZSGDZ!!!=",shape(data_one_tile_3d)
     call netcdf_err(error, 'WRITING DZ RECORD' )
   endif
...
!--- define dimension
 ...
 !--- define field
     ...
     error = nf90_def_var(ncid, 'sphum', NF90_DOUBLE,(/dim_lon,dim_lat,dim_lev,dim_time/), id_sphum)
     ...
     error = nf90_def_var(ncid, 'liq_wat',NF90_DOUBLE,(/dim_lon,dim_lat,dim_lev,dim_time/), id_liq_wat)
     ...
     if (gfdl_mp) then
       error = nf90_def_var(ncid, 'rainwat',NF90_DOUBLE,(/dim_lon,dim_lat,dim_lev,dim_time/), id_rwmr)
       ...
       error = nf90_def_var(ncid, 'ice_wat',NF90_DOUBLE,(/dim_lon,dim_lat,dim_lev,dim_time/), id_icmr)
       ...
       error = nf90_def_var(ncid, 'snowwat',NF90_DOUBLE,(/dim_lon,dim_lat,dim_lev,dim_time/), id_snmr)
       ...
       error = nf90_def_var(ncid, 'graupel',NF90_DOUBLE,(/dim_lon,dim_lat,dim_lev,dim_time/), id_grle)
       ...
       error = nf90_def_var(ncid,'cld_amt',NF90_DOUBLE,(/dim_lon,dim_lat,dim_lev,dim_time/), id_clda)
       call netcdf_err(error, 'WRITING cld_amt' )
       error = nf90_put_att(ncid, id_clda, "long_name", "cld_amt")
       error = nf90_put_att(ncid, id_clda, "units", "none")
       call netcdf_err(error, 'WRITING cld_amt' )

     endif

     error = nf90_def_var(ncid, 'o3mr', NF90_DOUBLE,(/dim_lon,dim_lat,dim_lev,dim_time/),id_o3mr)
...
!    graupel
     ...
     if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__))&
       call mpi_abort
!     print*,"GRAUPELLLLLLLLLL!!!!=",data_one_tile_3d
...
!    cloud amount
     ...
     if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__))&
       call mpi_abort
!     print*,"CLOUDDDDDDDDDDDDD!!!!=",data_one_tile_3d
     if (localpet == 0) then
       call flip(data_one_tile_3d, i_target, j_target, lev_target)
       dum3d(:,:,:) = data_one_tile_3d(i_start:i_end,j_start:j_end,:)
!       print*,"CLOUDDDDDDDDDDDDD!!!!=",dum3d
       error = nf90_put_var( ncid, id_clda, dum3d)
       call netcdf_err(error, 'WRITING CLOUD AMT RECORD' )
     endif
...
!-------------------------------------------------------------------------------
 ...
 use surface, only               : canopy_mc_target_grid,  &
                                   ...
                                   vegstate_type_target_grid, &
                                   zorlstate_target_grid, &
                                   tseastate_target_grid, &
                                   canopystate_target_grid, &
                                   slcstate_target_grid, &
                                   smcstate_target_grid
...
!   call ESMF_FieldGather(z0_target_grid, data_one_tile, rootPet=0, tile=tile, rc=error)
   call ESMF_FieldGather(zorlstate_target_grid, data_one_tile, rootPet=0, tile=tile, rc=error)
   if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
      call mpi_abort
...
!   call ESMF_FieldGather(vegstate_type_target_grid, data_one_tile, rootPet=0, tile=tile, rc=error)
   if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
      call mpi_abort
...
!   call ESMF_FieldGather(skin_temp_target_grid, data_one_tile, rootPet=0, tile=tile, rc=error)
   call ESMF_FieldGather(tseastate_target_grid, data_one_tile, rootPet=0, tile=tile, rc=error)
   if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
      call mpi_abort
...
!   call ESMF_FieldGather(canopy_mc_target_grid, data_one_tile, rootPet=0, tile=tile, rc=error)
   call ESMF_FieldGather(canopystate_target_grid, data_one_tile, rootPet=0, tile=tile, rc=error)
   if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
      call mpi_abort
...
!   call ESMF_FieldGather(soilm_tot_target_grid, data_one_tile_3d, rootPet=0, tile=tile, rc=error)
   call ESMF_FieldGather(smcstate_target_grid, data_one_tile_3d, rootPet=0, tile=tile, rc=error)
   if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
      call mpi_abort
...
!   call ESMF_FieldGather(soilm_liq_target_grid, data_one_tile_3d, rootPet=0, tile=tile, rc=error)
   call ESMF_FieldGather(slcstate_target_grid, data_one_tile_3d, rootPet=0, tile=tile, rc=error)
   if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
      call mpi_abort
