...
!--- define dimension
     ...
     error = nf90_def_dim(ncid, 'levp', levp_target, dim_levp)
     call netcdf_err(error, 'DEFINING LEVP DIMENSION' )
     error = nf90_def_dim(ncid, 'ntracer', ntracer_target, dim_ntracer)
     call netcdf_err(error, 'DEFINING NTRACER DIMENSION' )

 !--- define field
     error = nf90_def_var(ncid, 'lon', NF90_FLOAT, (/dim_lon,dim_lat/), id_lon)
     error = nf90_put_att(ncid, id_lon, "long_name", "longitude")
     error = nf90_put_att(ncid, id_lon, "units", "none")
     call netcdf_err(error, 'DEFINING LON FIELD' )

     error = nf90_def_var(ncid, 'lat', NF90_FLOAT, (/dim_lon,dim_lat/), id_lat)
     error = nf90_put_att(ncid, id_lat, "long_name", "latitude")
     error = nf90_put_att(ncid, id_lat, "units", "none")
     call netcdf_err(error, 'DEFINING LAT FIELD' )

     error = nf90_def_var(ncid, 'lon_s', NF90_FLOAT, (/dim_lon,dim_latp/), id_lon_s)
     error = nf90_put_att(ncid, id_lon_s, "long_name", "longitude south")
     error = nf90_put_att(ncid, id_lon_s, "units", "none")
     call netcdf_err(error, 'DEFINING LON_s FIELD' )

     error = nf90_def_var(ncid, 'lat_s', NF90_FLOAT, (/dim_lon,dim_latp/), id_lat_s)
     error = nf90_put_att(ncid, id_lat_s, "long_name", "latitude south")
     error = nf90_put_att(ncid, id_lat_s, "units", "none")
     call netcdf_err(error, 'DEFINING LAT_s FIELD' )


     error = nf90_def_var(ncid, 'lon_w', NF90_FLOAT, (/dim_lonp,dim_lat/), id_lon_w)
     error = nf90_put_att(ncid, id_lon_w,"long_name", "longitude west")
     error = nf90_put_att(ncid, id_lon_w,"units", "none")
     call netcdf_err(error, 'DEFINING LON_w FIELD' )

     error = nf90_def_var(ncid, 'lat_w', NF90_FLOAT, (/dim_lonp,dim_lat/), id_lat_w)
     error = nf90_put_att(ncid, id_lat_w, "long_name", "latitude west")
     error = nf90_put_att(ncid, id_lat_w, "units", "none")
     call netcdf_err(error, 'DEFINING LAT_w FIELD' )


     error = nf90_def_var(ncid, 'lon_c', NF90_FLOAT, (/dim_lonp,dim_latp/), id_lon_c)
     error = nf90_put_att(ncid, id_lon_c, "long_name", "longitude corners")
     error = nf90_put_att(ncid, id_lon_c, "units", "none")
     call netcdf_err(error, 'DEFINING LON_c FIELD' )

     error = nf90_def_var(ncid, 'lat_c', NF90_FLOAT, (/dim_lonp,dim_latp/), id_lat_c)
     error = nf90_put_att(ncid, id_lat_c, "long_name", "latitude corners")
     error = nf90_put_att(ncid, id_lat_c, "units", "none")
     call netcdf_err(error, 'DEFINING LAT_c FIELD' )
     ...
     call netcdf_err(error, 'WRITING SPEC HUM' )
     error = nf90_def_var(ncid, 'o3mr', NF90_FLOAT, (/dim_lon,dim_lat,dim_lev/), id_o3mr)
     call netcdf_err(error, 'WRITING OZONE' )
     error = nf90_def_var(ncid, 'liq_wat', NF90_FLOAT, (/dim_lon,dim_lat,dim_lev/), id_liq_wat)
     call netcdf_err(error, 'WRITING LIQUID WATER' )
     if (gfdl_mp) then
       error = nf90_def_var(ncid, 'rwmr', NF90_FLOAT, (/dim_lon,dim_lat,dim_lev/), id_rwmr)
       call netcdf_err(error, 'WRITING RAIN WATER' )
       error = nf90_def_var(ncid, 'icmr', NF90_FLOAT, (/dim_lon,dim_lat,dim_lev/), id_icmr)
       call netcdf_err(error, 'WRITING ICE WATER' )
       error = nf90_def_var(ncid, 'snmr', NF90_FLOAT, (/dim_lon,dim_lat,dim_lev/), id_snmr)
       call netcdf_err(error, 'WRITING SNOW WATER' )
       error = nf90_def_var(ncid, 'grle', NF90_FLOAT, (/dim_lon,dim_lat,dim_lev/), id_grle)
     ...
     error = nf90_def_var(ncid, 'u', NF90_FLOAT, (/dim_lon,dim_latp,dim_lev/),id_u)
     error = nf90_put_att(ncid, id_u, "long_name", "u wind")
     error = nf90_put_att(ncid, id_u, "units", "m/s")
     call netcdf_err(error, 'WRITING U' )

     error = nf90_def_var(ncid, 'v', NF90_FLOAT, (/dim_lonp,dim_lat,dim_lev/),id_v)
...
!  longitude
   ...
   if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
      call mpi_abort
   print*,"SG SG SG LONGITUDES SG SG SG", data_one_tile
   ...
   print*,"- CALL FieldGather FOR TARGET GRID LONGITUDE_w FOR TILE: ", tile
   call ESMF_FieldGather(longitude_w_target_grid, data_one_tile_ip1j, rootPet=0, tile=tile, rc=error)
   if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
      call mpi_abort
   print*,"SG1 SG1 SG1 SG1 datashape=", shape(data_one_tile_ip1j)

   if (localpet == 0) then
     dum2d_ip1j(:,:) = data_one_tile_ip1j(i_start:ip1_end, j_start:j_end)
     error = nf90_put_var( ncid, id_lon_w, dum2d_ip1j(:,:))
     call netcdf_err(error, 'WRITING LONGITUDE_w RECORD' )
   endif

   print*,"- CALL FieldGather FOR TARGET GRID LONGITUDE_s FOR TILE: ", tile
   call ESMF_FieldGather(longitude_s_target_grid, data_one_tile_ijp1, rootPet=0, tile=tile, rc=error)
   if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
      call mpi_abort

   if (localpet == 0) then
     dum2d_ijp1(:,:) = data_one_tile_ijp1(i_start:i_end, j_start:jp1_end)
     error = nf90_put_var( ncid, id_lon_s, dum2d_ijp1(:,:))
     call netcdf_err(error, 'WRITING LONGITUDE_s RECORD' )
   endif
   ...
   if (localpet == 0) then
     ...
     error = nf90_put_var( ncid, id_lon_c, dum2d_ip1jp1(:,:))
     call netcdf_err(error, 'WRITING LONGITUDE_c RECORD' )
...
!!  latitude
   ...
   print*,"- CALL FieldGather FOR TARGET GRID LATITUDE_w FOR TILE: ", tile
   call ESMF_FieldGather(latitude_w_target_grid, data_one_tile_ip1j, rootPet=0, tile=tile, rc=error)
   if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
      call mpi_abort

   if (localpet == 0) then
     dum2d_ip1j(:,:) = data_one_tile_ip1j(i_start:ip1_end, j_start:j_end)
     error = nf90_put_var( ncid, id_lat_w, dum2d_ip1j(:,:))
     call netcdf_err(error, 'WRITING LATITUDE_w RECORD' )
   endif

   print*,"- CALL FieldGather FOR TARGET GRID LATITUDE_s FOR TILE: ", tile
   call ESMF_FieldGather(latitude_s_target_grid, data_one_tile_ijp1, rootPet=0, tile=tile, rc=error)
   if(ESMF_logFoundError(rcToCheck=error,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
      call mpi_abort

   if (localpet == 0) then
     dum2d_ijp1(:,:) = data_one_tile_ijp1(i_start:i_end, j_start:jp1_end)
     error = nf90_put_var( ncid, id_lat_s, dum2d_ijp1(:,:))
     call netcdf_err(error, 'WRITING LATITUDE_s RECORD' )
   endif
   ...
   if (localpet == 0) then
     dum2d_ip1jp1(:,:) = data_one_tile_ip1jp1(i_start:ip1_end, j_start:jp1_end)
     error = nf90_put_var( ncid, id_lat_c, dum2d_ip1jp1(:,:))
     call netcdf_err(error, 'WRITING LATITUDE_c RECORD' )
...
!  height
   ...
   if (localpet == 0) then
     allocate(dum3d(i_target_out,j_target_out,levp_target))
     allocate(data_one_tile_3d(i_target,j_target,levp_target))
   else
...
!   nlev=shape(u_w,3)
   ...
   if (localpet == 0) then
     ...
     do j=1,j_target
        do i=1,i_target+1
!           print*,"- SG SG SG SG SG ij for vdnew: ", i,j
!           print*,"- SG SG SG SG shape(lon_corner): ", shape(lon_corner)
           p1(1) = lon_corner(i,j)
           p1(2) = lat_corner(i,j)
           p2(1) = lon_corner(i,j+1)
           p2(2) = lat_corner(i,j+1)
!           print*,"- SG SG SG SG assigned p1 and p2: ", p1,p2
           ...
           do k=1,nlev
!              print*,"- SG SG SG SG ijk: ", i,j,k
!              print*,"- SG SG SG SG VD_inner_prod(e1,ex): ", u_w(i,j,k)*inner_prod(e1,ex)
!              print*,"- SG SG SG SG VD_inner_prod(e1,ey): ", v_w(i,j,k)*inner_prod(e1,ey)
              vdnew(i,j,k) = u_w(i,j,k)*inner_prod(e1,ex) + &
                             v_w(i,j,k)*inner_prod(e1,ey)
           enddo
        enddo
     enddo
!     print*,"- SG SG SG SG SG vdnew/UV_w=",  vdnew/(sqrt(u_w**2 + v_w**2)) 
! upoint south
     print*,"- SG SG SG between vdnew udnew"
     do j=1,j_target+1
        do i=1,i_target
!           print*,"- SG SG SG SG SG ij for udnew: ", i,j
           p1(1) = lon_corner(i,j)
           p1(2) = lat_corner(i,j)
           ...
           do k=1,nlev
!              print*,"- SG SG SG SG ijk: ", i,j,k
!              print*,"- SG SG SG SG UD_inner_prod(e1,ex): ", u_s(i,j,k)*inner_prod(e1,ex)
!              print*,"- SG SG SG SG UD_inner_prod(e1,ey): ", v_s(i,j,k)*inner_prod(e1,ey)
              udnew(i,j,k) = u_s(i,j,k)*inner_prod(e1,ex) + &
                             v_s(i,j,k)*inner_prod(e1,ey)
           enddo
        enddo
     enddo
!     print*,"- SG SG SG SG SG vdnew/UV_s=", udnew/(sqrt(u_s**2 + v_s**2))
!     print*,"- SG SG SG SG SG UVdnew/UV_s=",  sqrt(udnew**2+vdnew**2)/(sqrt(u_s**2 + v_s**2))      
...
!   print*,"SGSGSGSG u_s,u_w,v_s,v_w ",u_s,u_w,v_s,v_w
!   print*,"SGSGSGSG udnew, vdnew",udnew,vdnew
!-------------------------------------------------------------------------------
!-------------------------------------------------------------------------------
!-------------------------------------------------------------------------------
!-------------------------------------------------------------------------------
