 ...
 type(esmf_field), public           :: dz_target_grid
 type(esmf_field)                   :: dz_b4adj_target_grid
...
!-----------------------------------------------------------------------------------
 ...
 print*,"- CALL Field_Regrid FOR DZ."
 call ESMF_FieldRegrid(dz_input_grid, &
                       dz_b4adj_target_grid, &
                       routehandle=regrid_bl, &
                       termorderflag=ESMF_TERMORDER_SRCSEQ, rc=rc)
 if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) call mpi_abort
...
!-----------------------------------------------------------------------------------
 ...
 print*,"- CALL FieldCreate FOR TARGET GRID DZ BEFORE ADJUSTMENT."
 dz_b4adj_target_grid = ESMF_FieldCreate(target_grid, &
                                   typekind=ESMF_TYPEKIND_R8, &
                                   staggerloc=ESMF_STAGGERLOC_CENTER, &
                                   ungriddedLBound=(/1/), &
                                   ungriddedUBound=(/lev_input/), rc=rc)
...
!-----------------------------------------------------------------------------------
 ...
 print*,"- CALL FieldCreate FOR TARGET GRID DZ."
 dz_target_grid = ESMF_FieldCreate(target_grid, &
                                   typekind=ESMF_TYPEKIND_R8, &
                                   staggerloc=ESMF_STAGGERLOC_CENTER, &
                                   ungriddedLBound=(/1/), &
                                   ungriddedUBound=(/lev_target/), rc=rc)
...
!C$$$
 ...
 real(esmf_kind_r8), pointer     :: dz1ptr(:,:,:)    ! input dz
 real(esmf_kind_r8), pointer     :: dz2ptr(:,:,:)    ! output dz 
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 print*,"- CALL FieldGet FOR DZ."
 call ESMF_FieldGet(dz_b4adj_target_grid, &
                    farrayPtr=dz1ptr, rc=rc)
 if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
         call mpi_abort

 print*,"- CALL FieldGet FOR ADJUSTED DZ."
 call ESMF_FieldGet(dz_target_grid, &
                    farrayPtr=dz2ptr, rc=rc)
 if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__))&
         call mpi_abort
 ...
 if (gfdl_mp) then
   ...
   if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
         call mpi_abort
!   print*,"-SG SG SG atmosphere.F90 GRAUPEL.",grle1ptr
!   print*,"-SG SG SG atmosphere.F90 CLOUD.",clda1ptr
...
! tracer.  So add one extra third dimension to these 3-d arrays.
 ...
 ALLOCATE(C1(CLB(1):CUB(1),CLB(2):CUB(2),LEV_INPUT,NTRACER_INPUT+6))
 ALLOCATE(C2(CLB(1):CUB(1),CLB(2):CUB(2),LEV_TARGET,NTRACER_TARGET+6))
 ...
 DO K=1,LEV_INPUT
   ...
   DO J=CLB(2),CUB(2)
     ...
     C1(I,J,K,4) =  DZ1PTR(I,J,K)
     C1(I,J,K,5) =  T1PTR(I,J,K)
     C1(I,J,K,6) =  Q1PTR(I,J,K)
   ENDDO
   ENDDO
...
! hardwire tracers to three for now (plus one extra for 'z' wind).
 ...
 DO K=1,LEV_INPUT
   DO I=CLB(1),CUB(1)
   DO J=CLB(2),CUB(2)
     C1(I,J,K,7) = CLW1PTR(I,J,K)
     C1(I,J,K,8) = O31PTR(I,J,K)
     C1(I,J,K,9) = WIND1PTR(I,J,K,3)  ! 'z' component of horiz wind
   ENDDO
   ENDDO
 ENDDO
 ...
 IF (GFDL_MP) THEN
   DO K=1,LEV_INPUT
     DO I=CLB(1),CUB(1)
     DO J=CLB(2),CUB(2)
       C1(I,J,K,10) =  RWMR1PTR(I,J,K)
       C1(I,J,K,11) = SNMR1PTR(I,J,K)
       C1(I,J,K,12) = ICMR1PTR(I,J,K)
       C1(I,J,K,13) = GRLE1PTR(I,J,K)
       C1(I,J,K,14) = CLDA1PTR(I,J,K)
     ENDDO
     ENDDO
   ENDDO
 ENDIF
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 DO K=1,LEV_TARGET
   ...
   DO J=CLB(2),CUB(2)
     ...
     DZ2PTR(I,J,K)=C2(I,J,K,4)
     DZ=Z2(I,J,K)-Z1(I,J,1)
     IF(DZ.GE.0) THEN
       T2PTR(I,J,K)=C2(I,J,K,5)
       Q2PTR(I,J,K)=C2(I,J,K,6)
     ELSE
...
!  if (localpet==0) print*,'after  vintg p/t:',k,p2ptr(1,1,k),t2ptr(1,1,k)
 ...
 DO K=1,LEV_TARGET
   DO I=CLB(1),CUB(1)
   DO J=CLB(2),CUB(2)
     CLW2PTR(I,J,K)=C2(I,J,K,7)
     O32PTR(I,J,K)=C2(I,J,K,8)
     WIND2PTR(I,J,K,3)=C2(I,J,K,9)
   ENDDO
   ENDDO
 ENDDO
 ...
 IF (GFDL_MP) THEN
   DO K=1,LEV_TARGET
     DO I=CLB(1),CUB(1)
     DO J=CLB(2),CUB(2)
       RWMR2PTR(I,J,K) = C2(I,J,K,10)
       SNMR2PTR(I,J,K) = C2(I,J,K,11)
       ICMR2PTR(I,J,K) = C2(I,J,K,12)
       GRLE2PTR(I,J,K) = C2(I,J,K,13)
       CLDA2PTR(I,J,K) = C2(I,J,K,14)
     ENDDO
     ENDDO
   ENDDO
 ENDIF
...
!C  INPUT COORDINATE IS MONOTONICALLY DESCENDING.                       
 ...
 call ESMF_FieldDestroy(dz_b4adj_target_grid, rc=rc)
 ...
 call ESMF_FieldDestroy(dz_target_grid, rc=rc)
