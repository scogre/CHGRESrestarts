...
! fields associated with the atmospheric model.
 ...
 type(esmf_field), public        :: dz_input_grid
...
! fields associated with the land-surface model.
 ...
 type(esmf_field), public        :: slcstate_input_grid
...
! moved from  static_data.F90 for restarts
 ...
 type(esmf_field), public           :: mxsno_albedo_input_grid
 type(esmf_field), public           :: slope_type_input_grid
! type(esmf_field), public           :: soil_type_input_grid
 type(esmf_field), public           :: substrate_temp_input_grid
...
!---------------------------------------------------------------------------
 ...
 print*,"- CALL FieldCreate FOR INPUT GRID DZ."
 dz_input_grid = ESMF_FieldCreate(input_grid, &
                                   typekind=ESMF_TYPEKIND_R8, &
                                   staggerloc=ESMF_STAGGERLOC_CENTER, &
                                   ungriddedLBound=(/1/), &
                                   ungriddedUBound=(/lev_input/), rc=rc)
...
!!!
   if (localpet == 0) then
     error=nf90_inq_varid(ncid, 'DZ', id_var)
     call netcdf_err(error, 'reading field id' )
     error=nf90_get_var(ncid, id_var, data_one_tile_3d)
     call netcdf_err(error, 'reading field' )
     call flip(data_one_tile_3d, i_input, j_input, lev_input)
     print*,'dz b4 flip ',data_one_tile_3d(1,1,:)
   endif

   print*,"- CALL FieldScatter FOR INPUT GRID DZ."
   call ESMF_FieldScatter(dz_input_grid, data_one_tile_3d, rootpet=0,tile=tile, rc=rc)
   if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__))&
      call mpi_abort
!!!!
...
!---------------------------------------------------------------------------
 ...
 print*,"- CALL FieldCreate FOR INPUT max_veg_greenness."
 ...
 print*,"- CALL FieldCreate FOR INPUT min_veg_greenness."
 ...
 print*,"- CALL FieldCreate FOR INPUT mxsno_albedo."
 mxsno_albedo_input_grid = ESMF_FieldCreate(input_grid, &
                                   typekind=ESMF_TYPEKIND_R8, &
                                   staggerloc=ESMF_STAGGERLOC_CENTER, rc=rc)
 if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
    call mpi_abort

 print*,"- CALL FieldCreate FOR INPUT slope_type."
 slope_type_input_grid = ESMF_FieldCreate(input_grid, &
                                   typekind=ESMF_TYPEKIND_R8, &
                                   staggerloc=ESMF_STAGGERLOC_CENTER, rc=rc)
 if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
    call mpi_abort

! print*,"- CALL FieldCreate FOR INPUT soil_type."
! soil_type_input_grid = ESMF_FieldCreate(input_grid, &
!                                   typekind=ESMF_TYPEKIND_R8, &
!                                   staggerloc=ESMF_STAGGERLOC_CENTER, rc=rc)
! if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
!    call mpi_abort
 ...
 print*,"- CALL FieldCreate FOR INPUT substrate_temp."
 substrate_temp_input_grid = ESMF_FieldCreate(input_grid, &
                                   typekind=ESMF_TYPEKIND_R8, &
                                   staggerloc=ESMF_STAGGERLOC_CENTER, rc=rc)
 if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
    call mpi_abort

 print*,"- CALL FieldCreate FOR INPUT veg_greenness."
...
! print*,"- CALL FieldCreate FOR INPUT veg_type."
! veg_type_input_grid = ESMF_FieldCreate(input_grid, &
!                                   typekind=ESMF_TYPEKIND_R8, &
!                                   staggerloc=ESMF_STAGGERLOC_CENTER, rc=rc)
! if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
...
! alvsf
  if (localpet == 0) then
    call read_fv3_grid_data_netcdf('alvsf', tile, idim_input, jdim_input, &
                                   lsoil_input, data_dir_input_grid, &
                                   sfcdata=data_one_tile)
  endif
...
! alvwf
  if (localpet == 0) then
    call read_fv3_grid_data_netcdf('alvwf', tile, idim_input, jdim_input, &
                                   lsoil_input, data_dir_input_grid, &
                                   sfcdata=data_one_tile)
  endif
...
! alnsf
  if (localpet == 0) then
    call read_fv3_grid_data_netcdf('alnsf', tile, idim_input, jdim_input, &
                                   lsoil_input, data_dir_input_grid, &
                                   sfcdata=data_one_tile)
  endif
...
! alnwf
  if (localpet == 0) then
    call read_fv3_grid_data_netcdf('alnwf', tile, idim_input, jdim_input, &
                                   lsoil_input, data_dir_input_grid, &
                                   sfcdata=data_one_tile)
  endif
...
! facsf
  if (localpet == 0) then
    call read_fv3_grid_data_netcdf('facsf', tile, idim_input, jdim_input, &
                                   lsoil_input, data_dir_input_grid, &
                                   sfcdata=data_one_tile)
  endif
...
! facwf
  if (localpet == 0) then
    call read_fv3_grid_data_netcdf('facwf', tile, idim_input, jdim_input, &
                                   lsoil_input, data_dir_input_grid, &
                                   sfcdata=data_one_tile)
  endif
...
! max_veg_greenness
  if (localpet == 0) then
    call read_fv3_grid_data_netcdf('shdmax', tile, idim_input, jdim_input, &
                                   lsoil_input, data_dir_input_grid, &
                                   sfcdata=data_one_tile)
  endif
  print*,"- CALL FieldScatter FOR INPUT GRID max_veg_greenness."
...
! min_veg_greenness
  if (localpet == 0) then
    call read_fv3_grid_data_netcdf('shdmin', tile, idim_input, jdim_input, &
                                   lsoil_input, data_dir_input_grid, &
                                   sfcdata=data_one_tile)
  endif
  print*,"- CALL FieldScatter FOR INPUT GRID min_veg_greenness."
...
! mxsno_albedo
  if (localpet == 0) then
    call read_fv3_grid_data_netcdf('snoalb', tile, idim_input, jdim_input, &
                                   lsoil_input, data_dir_input_grid, &
                                   sfcdata=data_one_tile)
  endif
  print*,"- CALL FieldScatter FOR INPUT GRID mxsno_albedo."
  call ESMF_FieldScatter(mxsno_albedo_input_grid, data_one_tile, rootpet=0, tile=tile, rc=rc)
  if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
     call mpi_abort

! slope_type
  if (localpet == 0) then
    call read_fv3_grid_data_netcdf('slope', tile, idim_input, jdim_input, &
                                   lsoil_input, data_dir_input_grid, &
                                   sfcdata=data_one_tile)
  endif
  print*,"- CALL FieldScatter FOR INPUT GRID slope."
  call ESMF_FieldScatter(slope_type_input_grid, data_one_tile, rootpet=0, tile=tile, rc=rc)
  if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
     call mpi_abort
...
!                                   sfcdata=data_one_tile)
!  endif
!  print*,"- CALL FieldScatter FOR INPUT GRID stype."
...
! substrate temp
  if (localpet == 0) then
    call read_fv3_grid_data_netcdf('tg3', tile, idim_input, jdim_input, &
                                   lsoil_input, data_dir_input_grid, &
                                   sfcdata=data_one_tile)
  endif
  print*,"- CALL FieldScatter FOR INPUT GRID tg3."
  call ESMF_FieldScatter(substrate_temp_input_grid, data_one_tile, rootpet=0, tile=tile, rc=rc)
  if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
     call mpi_abort
...
! Vegetation greenness
  if (localpet == 0) then
    call read_fv3_grid_data_netcdf('vfrac', tile, idim_input, jdim_input, &
                                   lsoil_input, data_dir_input_grid, &
                                   sfcdata=data_one_tile)
  endif
  print*,"- CALL FieldScatter FOR INPUT GRID VFRAC."
...
!                                  sfcdata=data_one_tile)
 ...
 call ESMF_FieldDestroy(dz_input_grid, rc=rc)
 if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
    call mpi_abort
 ...
 call ESMF_FieldDestroy(ffhh_input_grid, rc=rc)
 if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
    call mpi_abort
...
! call ESMF_FieldDestroy(slcstate_input_grid, rc=rc)
! if(ESMF_logFoundError(rcToCheck=rc,msg=ESMF_LOGERR_PASSTHRU,line=__line__,file=__file__)) &
!    call mpi_abort
